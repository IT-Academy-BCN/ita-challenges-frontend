...
export class ChallengeHeaderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const savedChallenge = JSON.parse(
      localStorage.getItem("challengeStarted") ?? "{}"
    ) as { id?: string; started?: boolean };

    console.log(localStorage.getItem("challengeStarted"));

    if (
      savedChallenge.id === this.idChallenge &&
      savedChallenge?.started === true
    ) {
  ...
  async onStartChallenge(): Promise<void> {
    ...
    localStorage.setItem(
      "challengeStarted",
      JSON.stringify({ id: this.idChallenge, started: true })
    );
    ...
    try {
      await this.router.navigate([
        `/ita-challenge/challenges/${this.idChallenge}/start`,
      ]);
    } catch (error) {
